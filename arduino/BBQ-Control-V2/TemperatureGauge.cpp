...
void TemperatureGauge::draw(Widgets&w,float core,float target,bool valid,float arc,bool localTarget){
 if(!w.region(12,42,280,240))return;
 w.roundRect(0,0,280,240,7,Theme::Panel);
 w.text(12,8,"KERNTEMPERATUR",SmallFont,Theme::Blue);
 float progress=valid&&isfinite(target)&&target>0?std::max(0.f,std::min(1.f,core/target)):0;
 if(arc>=0)progress=arc;
 // 270 degree arc, clockwise from lower left, with orange/red near the target.
 for(uint32_t p:RingPixels){
  unsigned angle=(p>>16)&255;
  w.pixel(29+(p&255),29+((p>>8)&255),valid&&angle<=progress*255?(angle<205?Theme::Green:angle<240?Theme::Orange:Theme::Red):Theme::Track,(p>>24)/255.f);
 }
 ...
 w.centered(140,77,value,HeroFont,Theme::White);
 w.circle(130,166,2,1.5,Theme::White);w.text(137,156,"C",LabelFont,Theme::White);
 w.centered(140,180,isfinite(target)?String(localTarget?"LOKAL: ":"ZIEL: ")+String(target,0)+" C":"ZIEL: --",LabelFont,Theme::Blue);
 ...
 w.centered(140,216,label,SmallFont,tone);
 w.present();
}
